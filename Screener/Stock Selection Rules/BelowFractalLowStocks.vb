...
Public Class BelowFractalLowStocks
            ...
            While tradingDate <= endDate
                ...
                If atrStockList IsNot Nothing AndAlso atrStockList.Count > 0 Then
                    ...
                    For Each runningStock In atrStockList.Keys
                        _canceller.Token.ThrowIfCancellationRequested()
                        Dim eodPayload As Dictionary(Of Date, Payload) = _cmn.GetRawPayload(Common.DataBaseTable.EOD_POSITIONAL, runningStock, previousTradingDay.AddYears(-1), previousTradingDay)
                        If eodPayload IsNot Nothing AndAlso eodPayload.Count > 0 Then
                            ...
                            Indicator.FractalBands.CalculateFractal(eodPayload, fractalHighPayload, fractalLowPayload)

                            Dim previousDayPayload As Payload = eodPayload.LastOrDefault.Value
                            If previousDayPayload.Open < fractalLowPayload(previousDayPayload.PayloadDate) AndAlso
                                previousDayPayload.Close < fractalLowPayload(previousDayPayload.PayloadDate) AndAlso
                                previousDayPayload.CandleColor = Color.Red Then
