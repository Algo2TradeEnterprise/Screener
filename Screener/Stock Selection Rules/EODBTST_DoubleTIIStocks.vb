...
Public Class EODBTST_DoubleTIIStocks
                ...
                If atrStockList IsNot Nothing AndAlso atrStockList.Count > 0 Then
                    ...
                    For Each runningStock In atrStockList.Keys
                        ...
                        If eodPayload IsNot Nothing AndAlso eodPayload.Count > 200 AndAlso eodPayload.ContainsKey(tradingDate.Date) Then
                            ...
                            Dim tii5Payload As Dictionary(Of Date, Decimal) = Nothing
                            Indicator.TrendIntensityIndex.CalculateTII(Payload.PayloadFields.Close, 5, 5, eodPayload, tii5Payload, Nothing)
                            Dim tii15Payload As Dictionary(Of Date, Decimal) = Nothing
                            Indicator.TrendIntensityIndex.CalculateTII(Payload.PayloadFields.Close, 15, 15, eodPayload, tii15Payload, Nothing)

                            If tii15Payload(currentDayCandle.PayloadDate) = 100 AndAlso
                                tii5Payload(currentDayCandle.PayloadDate) = 0 Then
                                Dim validSignal As Boolean = True
                                For Each runningPayload In eodPayload.OrderByDescending(Function(x)
                                                                                            Return x.Key
                                                                                        End Function)
                                    If runningPayload.Key < currentDayCandle.PayloadDate Then
                                        If tii15Payload(runningPayload.Key) = 100 Then
                                            If tii5Payload(runningPayload.Key) = 0 Then
                                                validSignal = False
                                                Exit For
                                            End If
                                        Else
