...
Public Class EODVolumeEMAStocks
            ...
            While tradingDate <= endDate
                ...
                If atrStockList IsNot Nothing AndAlso atrStockList.Count > 0 Then
                    ...
                    For Each runningStock In atrStockList.Keys
                        Dim eodPayload As Dictionary(Of Date, Payload) = _cmn.GetRawPayload(_eodTable, runningStock, tradingDate.AddDays(-200), tradingDate.AddDays(-1))
                        If eodPayload IsNot Nothing AndAlso eodPayload.Count > 30 Then
                            Dim emaPayload As Dictionary(Of Date, Decimal) = Nothing
                            Indicator.EMA.CalculateEMA(13, Payload.PayloadFields.Volume, eodPayload, emaPayload)

                            Dim lastCandle As Payload = eodPayload.LastOrDefault.Value
                            If lastCandle.Volume > 2 * emaPayload(lastCandle.PayloadDate) Then
                                If tempStockList Is Nothing Then tempStockList = New Dictionary(Of String, String())
                                tempStockList.Add(runningStock, {lastCandle.Volume, Math.Round(emaPayload(lastCandle.PayloadDate), 2)})
                            End If
                        End If
                    Next
                    If tempStockList IsNot Nothing AndAlso tempStockList.Count > 0 Then
                        ...
                        For Each runningStock In tempStockList
                            ...
                            row("Volume") = runningStock.Value(0)
                            row("EMA") = runningStock.Value(1)
