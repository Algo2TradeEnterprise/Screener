...
Public Class HighestATRAtTheMoneyOptions
                        ...
                        If futureIntradayPayload IsNot Nothing AndAlso futureIntradayPayload.Count > 0 Then
                            ...
                            If optionData IsNot Nothing AndAlso optionData.Count > 0 Then
                                ...
                                For Each runningStock In optionData.Values
                                    ...
                                    Dim eodPayload As Dictionary(Of Date, Payload) = _cmn.GetRawPayloadForSpecificTradingSymbol(Common.DataBaseTable.EOD_Futures_Options, runningStock.TradingSymbol, previousTradingDay.AddYears(-1), previousTradingDay)
                                    If eodPayload IsNot Nothing AndAlso eodPayload.Count > 0 Then
                                        ...
                                        stockData(strikePrice).Add(intrumentType, eodPayload)
                                    End If
                                Next
                                If stockData IsNot Nothing AndAlso stockData.Count > 0 Then
                                    Dim usedStocklist As Dictionary(Of String, Tuple(Of Payload, Decimal)) = Nothing
                                    For Each runningPayload In futureIntradayPayload
                                        ...
                                        If strikePricesToCheck IsNot Nothing AndAlso strikePricesToCheck.Count > 0 Then
                                            Dim strikePayloads As Dictionary(Of String, Tuple(Of Payload, Decimal)) = New Dictionary(Of String, Tuple(Of Payload, Decimal))
                                            For Each runningStrike In strikePricesToCheck
                                                If stockData.ContainsKey(runningStrike) Then
                                                    For Each instrumentType In stockData(runningStrike).Values
                                                        If instrumentType.ContainsKey(previousTradingDay.Date) Then
                                                            Dim previousDayCandle As Payload = instrumentType(previousTradingDay.Date)
                                            ...
                                            If strikePayloads IsNot Nothing AndAlso strikePayloads.Count > 0 Then
                                                ...
                                                For Each optionPayload In strikePayloads.OrderByDescending(Function(x)
                                                                                                               Return x.Value.Item2
                                                                                                           End Function)
                                                    Dim intrumentType As String = optionPayload.Value.Item1.TradingSymbol.Substring(optionPayload.Value.Item1.TradingSymbol.Count - 2).Trim
                                                    If intrumentType = "CE" AndAlso Not ceDone Then
                                                        If usedStocklist Is Nothing Then usedStocklist = New Dictionary(Of String, Tuple(Of Payload, Decimal))
                                                        usedStocklist.Add(optionPayload.Value.Item1.TradingSymbol, optionPayload.Value)
                                                        ceDone = True
                                                    ElseIf intrumentType = "PE" AndAlso Not peDone Then
                                                        If usedStocklist Is Nothing Then usedStocklist = New Dictionary(Of String, Tuple(Of Payload, Decimal))
                                                        usedStocklist.Add(optionPayload.Value.Item1.TradingSymbol, optionPayload.Value)
                                                        peDone = True
                                                    End If
                                                    If ceDone AndAlso peDone Then Exit For
                                                Next
                                            End If
                                        End If
                                        Exit For
                                    ...
                                    If usedStocklist IsNot Nothing AndAlso usedStocklist.Count > 0 Then
                                        For Each runningPayload In usedStocklist
                                            Dim intrumentType As String = runningPayload.Value.Item1.TradingSymbol.Substring(runningPayload.Value.Item1.TradingSymbol.Count - 2).Trim
                                            Dim row As DataRow = ret.NewRow
                                            row("Date") = tradingDate.ToString("dd-MM-yyyy")
                                            row("Trading Symbol") = runningPayload.Value.Item1.TradingSymbol
                                            row("Lot Size") = lotSize
                                            row("Puts_Calls") = intrumentType
                                            row("Price") = runningPayload.Value.Item1.Close
                                            row("ATR") = Math.Round(runningPayload.Value.Item2, 4)
                                            row("Time") = "09:15:00"
                                            ret.Rows.Add(row)
                                        Next
                                    End If
