...
Public Class EODEMACrossoverStocks
                ...
                If atrStockList IsNot Nothing AndAlso atrStockList.Count > 0 Then
                    ...
                    For Each runningStock In atrStockList.Keys
                        ...
                        If eodPayload IsNot Nothing AndAlso eodPayload.Count > 30 AndAlso eodPayload.ContainsKey(tradingDate.Date) Then
                            Dim emaHighPayload As Dictionary(Of Date, Decimal) = Nothing
                            Indicator.EMA.CalculateEMA(5, Payload.PayloadFields.High, eodPayload, emaHighPayload)
                            Dim emaLowPayload As Dictionary(Of Date, Decimal) = Nothing
                            Indicator.EMA.CalculateEMA(5, Payload.PayloadFields.Low, eodPayload, emaLowPayload)

                            Dim direction As String = Nothing
                            Dim currentDayCandle As Payload = eodPayload(tradingDate.Date)
                            If currentDayCandle.Close >= emaHighPayload(currentDayCandle.PayloadDate) AndAlso
                                currentDayCandle.PreviousCandlePayload.Close <= emaHighPayload(currentDayCandle.PreviousCandlePayload.PayloadDate) Then
                                direction = "BUY"
                            ElseIf currentDayCandle.Close <= emalowPayload(currentDayCandle.PayloadDate) AndAlso
                                currentDayCandle.PreviousCandlePayload.Close >= emalowPayload(currentDayCandle.PreviousCandlePayload.PayloadDate) Then
                                direction = "SELL"
                            End If
